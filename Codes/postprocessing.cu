...
//	printf("allocatable_chunk is %d\n",allocatable_chunk);
	...
	//	printf("Going for bc_postprocess2\n");
		...
		//checkCudaErrors(cudaMemcpy2D(key_buff_out, sizeof(int)*g_n,  dummy_res_dist_out, pitch_dummy_res_dist_out, sizeof(int)*g_n, number_of_SMs, cudaMemcpyDeviceToHost));
		//checkCudaErrors(cudaMemcpy2D(value_buff_out, sizeof(int)*g_n,  res_seq_out, pitch_res_seq_out, sizeof(int)*g_n, number_of_SMs, cudaMemcpyDeviceToHost));

		/*for(int z=0 ;z<number_of_SMs;z++)	
		{
			std::cout<<" Initials "<<z<<std::endl;
			for(int q=0;q<n;q++)
			{
				std::cout<<key_buff_out[z*g_n + q]<<" "<<value_buff_out[z*g_n + q]<<std::endl;
			}
		}*/	
		...
		/*printf("PArtial betweenness centrality postprocessssss \n");
		for(int h=0; h<g_n; h++)
		{
			printf("%d = %f\n",h, tmp_bc_gpu[h]/2 );
		}*/
...
__global__ void bc_postprocess1(float* bc, int *sigma_ld, int *sigma_rd, int* d_ld, int* d_rd, int* S_ld,
	...
	int start_pos_offset, float *delta, size_t pitch_delta, int *res_dist, size_t pitch_res_dist, int *res_sigma, 
	size_t pitch_res_sigma, int *dummy_res_dist, int *res_seq)
{
	int tid = threadIdx.x;
	...
	//	printf("tid %d limit %d\n",tid,limit );
		...
		for(int k=threadIdx.x; k<n; k+=blockDim.x)
		{
			...
			/*if(res_sigma_row[k]==0)
			{
				printf("k %d res_sigma_row[%d] %d sigma_lrow[%d] %d sigma_rrow[%d] %d left node %d right node %d d_lrow[%d] %d d_rrow[%d] %d\n",k,k,res_sigma_row[k],k,sigma_lrow[k],k,sigma_rrow[k],left_nodes[blockIdx.x],right_nodes[blockIdx.x],k,d_lrow[k],k,d_rrow[k]);
			}*/	
		...
		if(threadIdx.x==0)
		{
			...
			left_nodes[blockIdx.x], right_nodes[blockIdx.x]);
		//	printf("dummy_res_dist_row[%d] = %d , res_seq_row[%d] = %d tid %d blockid %d\n",local_nodes[blockIdx.x],dummy_res_dist_row[local_nodes[blockIdx.x]],local_nodes[blockIdx.x],res_seq_row[local_nodes[blockIdx.x]],threadIdx.x,blockIdx.x);
			...
			/*if(left_nodes[blockIdx.x]==2)
			{
				printf("lposition is %d lmemsize %d start_pos_offset %d \n",position[left],lmemsize,start_pos_offset );
			}
			if(right_nodes[blockIdx.x]==2)
			{
				printf("rposition is %d lmemsize %d start_pos_offset %d \n",position[right],lmemsize,start_pos_offset );
			}*/	
		}	
		__syncthreads();
	}	
...
int *offset_Q, size_t pitch_offset_Q, int m, int *F, int *reachval, int limit, float *tmp_bc)
{
	...
	if(blockIdx.x < limit)		
	{
		...
		int *sigma_row = (int*)((char*)res_sigma + blockIdx.x*pitch_res_sigma);
